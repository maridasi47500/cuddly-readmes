...
Créer une app où les utilisateurs peuvent publier des posts, et où les
posts similaires sont automatiquement regroupés dans des « verses » —
sortes de collections thématiques ou narratives.
🧩 Modèle de données
ruby
...
Tu peux implémenter une méthode dans ton modèle Post ou dans un service
qui, à la création d’un post, recherche les posts existants avec du contenu
similaire :
ruby
...
def assign_to_verse
  ...
  self.update(verse: similar_post&.verse || Verse.create(title: "Verse
#{Verse.count + 1}"))
end
...
C'est une logique basique mais tu peux l'améliorer avec des outils comme:
   ...
   🧠 *Embeddings* + FAISS ou pgvector : si tu veux un vrai système de
...
Si tu veux, je peux t’aider à ajouter une API JSON, un système
d’authentification, ou à styliser tout ça avec Tailwind ou Hotwire. Tu veux
que ça ressemble à quoi côté expérience utilisateur ?
...
Rails ne peut pas directement appeler ces moteurs sans API, mais tu
pourrais :

   -

   Créer un *service externe* qui interroge une API (comme Bing Search ou
   une API NLP)
   -
...
<% if @post.verse %>
  <div class="verse-info">
    <h3>Vous explorez le Verse <%= @post.verse.id %>: "<%=
@post.verse.title %>"</h3>
...
Tu veux que je t’aide à calculer ce fameux pourcentage de similarité ? Ou à
simuler l’appel vers un moteur de recherche pour enrichir ton affichage ?
On peut rendre ça très intuitif pour l’utilisateur !
